...
export default function Home(){
  ...
  const riwayat = (JSON.parse(localStorage.getItem('mr.history')||'[]') as any[]).slice(0,10)
  return (
        ...
        <section>
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3">
            {riwayat.map((h:any,i:number)=>(
              <a key={i} href={`/manga/${h.slug}/${h.chapter}`} className="card">
                <img src={h.cover} alt={h.title} className="w-full aspect-[3/4] object-cover"/>
                <div className="p-2 text-sm">{h.title} <span className="muted">Ch {h.chapter}</span></div>
              </a>
            ))}
          </div>
        </section>
