...
export default function Reader(){
  ...
  const ch = s?.chapters.find(c=>c.number===chNum)

  React.useEffect(()=>{ if(s && ch){ history.push({ slug:s.slug, title:s.title, cover:s.cover, chapter: ch.number, at: Date.now() }) } },[slug, chapter])

  if(!s || !ch) return <main className="container-page">Not found</main>
  const prev = s.chapters.find(c=>c.number===chNum-1)
  const next = s.chapters.find(c=>c.number===chNum+1)
    ...
    <main>
        ...
        <div className="container-page flex flex-wrap gap-2 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <select className="btn" defaultValue={ch.number} onChange={(e)=>goto(Number(e.target.value))}>
              {s.chapters.map(c=> <option key={c.number} value={c.number}>Ep {c.number}</option>)}
            </select>
          </div>
        </div>
      </div>
      ...
      <div className={`container-page flex flex-col items-center ${mode==='webtoon' ? 'gap-0' : 'gap-3'}`}>
        {Array.from({length: ch.pages}).map((_,i)=>{
          ...
          const src = `/manga/${s.slug}/${ch.number}/${n}.png`
